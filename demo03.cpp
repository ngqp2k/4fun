...
vector< pair<string, int> > a; // Data <usename, score>
...
void checkPoint(int x1, int y1, int x2, int y2) {
	...
	if (res) {
		...
		char *charr = &st[0];
		outtextxy(10 + 80, getmaxy() - 50, charr);
...
void layoutGame(string user) {
	...
	// show name player
	settextjustify(0, 0);
	string st = "Nguoi choi: " + user;
	char *charr = &st[0];
	outtextxy(10, getmaxy() - 30, charr);

	// show score
	string st2 = "Diem: ";
	char *charr2 = &st2[0];
	outtextxy(10, getmaxy() - 50, charr2);
	...
	string stTime;
	int cntTime = 0;
	stTime = to_string(cntTime);
	charTime = &stTime[0];
	outtextxy(getmaxx() - 70, 50, charTime);
	clock_t aa, bb;
	aa = clock();
